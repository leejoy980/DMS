...
<body bgcolor="#F2F2F2">
	...
	<tr height="50">
		...
		<a href="#" onClick="history.go(-1);">[확인]</a>
			<c:if test="${answer != 'ReRe' }">
			<a href="movPage.do?doc_Type=answerDoc&boardType=${boardType}&doc_id=${detailDocVo.doc_id}&answer=${answer}&page=${page}">[답변]</a>
			</c:if>
		</c:if>
		<c:if test="${board != 'board'}">
		<a href="docList.do?page=${page}&listType=${listType}&user_id=${memberVo.user_id}">[확인]</a>
			...
			<c:if test="${listType == 'sendList'}">
			<a href="movPage.do?&doc_Type=modifyDoc&doc_id=${detailDocVo.doc_id}&title=${detailDocVo.title}&content=${detailDocVo.content}&attached=${detailDocVo.attached}">[수정]</a>
